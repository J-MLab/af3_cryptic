"""Script to calculate RMSD between crytic site residues in aligned structures and
their crystal structures. Moving residues can be found in pnas_table.csv.
For side chain RMSD calculation only.
Author: Maria L.
"""
...
def map_residues_by_coordinates(ref_chain, mobile_chain, threshold=7):
    """
    Maps residues from a reference chain to a mobile chain based on proximity and residue type.

    Args:
        ref_chain (list): List of residues in the reference chain.
        mobile_chain (list): List of residues in the mobile chain.
        threshold (float): Distance threshold for mapping residues.
    ...
    for model in structure:
        for chain in model:
            for residue in chain:
                ...
                elif residue.id[1] == startres or residue.id[1] == endres:
...
def calculate_rmsd(
    ...
    """
    Calculates RMSD between reference and mobile structures using all atoms that are present in both structures and in the same order.
    Args:
    ...
    for ref_res in interacting_residues:
        if ref_res in residue_map:
            ...
            print(mobile_res)
            # Find all atoms common to both residues
            common_atoms = set(atom.get_name() for atom in ref_res).intersection(
                set(atom.get_name() for atom in mobile_res)
            )

            # Add matched atoms for RMSD calculation
            for atom_name in common_atoms:
                ref_atoms.append(ref_res[atom_name])
                mobile_atoms.append(mobile_res[atom_name])
